...
-- 沙箱加了字段
alter table smart_contract add column SC_INNER_CLASS_FILES longtext after SC_CLASS_FILE;

--2021-02-05
-- block_cache 建索引
-- drop index block_block_id on block_cache;
create unique index block_block_id
	on block_cache (CHANNEL_ID, BLOCK_ID);

-- 20210205 block 建索引
drop index block_block_id on block;
create unique index block_block_id
	on block (CHANNEL_ID, BLOCK_ID);
