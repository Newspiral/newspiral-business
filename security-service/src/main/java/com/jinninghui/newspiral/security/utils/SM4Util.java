...
public class SM4Util extends GMBaseUtil {
    ...
    public static byte[] generateKey(int keySize) throws NoSuchAlgorithmException, NoSuchProviderException {
        KeyGenerator kg = KeyGenerator.getInstance(ALGORITHM_NAME, bouncyCastleProvider);
    ...
    private static byte[] doMac(String algorithmName, Key key, byte[] data) throws NoSuchProviderException,
        ...
        Mac mac = Mac.getInstance(algorithmName, bouncyCastleProvider);
        mac.init(key);
        mac.update(data);
        return mac.doFinal();
    }
    ...
    private static Cipher generateECBCipher(String algorithmName, int mode, byte[] key)
            ...
            InvalidKeyException {
        Cipher cipher = Cipher.getInstance(algorithmName, bouncyCastleProvider);
